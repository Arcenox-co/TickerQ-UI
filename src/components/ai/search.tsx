...
import { ChatBubble, Refresh, Search, Send, X } from 'iconoir-react';
    ...
    <form {...props} className={cn('flex items-start pe-2', props.className)} onSubmit={onStart}>
      ...
      {isLoading ? (
        <button
          ...
          onClick={stop}
        >
          <Refresh className="size-4 animate-spin text-fd-muted-foreground" />
          Abort Answer
        </button>
      ) : (
        <button
...
const suggestions = [
  'How do I get started with TickerQ?',
  'What is the difference between TimeTicker and CronTicker?',
  'How do I set up the Dashboard?',
  'How does job chaining work?',
  'How do I configure EF Core persistence?',
  'How do I handle errors and retries?',
];

function SuggestionChips({ sendMessage }: { sendMessage: UseChatHelpers<UIMessage>['sendMessage'] }) {
  return (
    <div className="size-full flex flex-col items-center justify-center gap-4 px-3">
      <div className="text-sm text-fd-muted-foreground/80 text-center">
        <ChatBubble className="size-5 mx-auto mb-2" />
        <p>Ask anything about TickerQ</p>
      </div>
      <div className="flex flex-wrap gap-2 justify-center">
        {suggestions.map((q) => (
          <button
            ...
            className="text-xs px-3 py-1.5 rounded-full border border-fd-border bg-fd-card text-fd-muted-foreground hover:bg-fd-muted hover:text-fd-foreground transition-colors text-left"
          >
            {q}
          </button>
        ))}
...
export function AISearchPanel() {
  ...
  return (
    <>
      <style>{`
        @keyframes ask-ai-open { from { translate: 100% 0; } to { translate: 0 0; } }
        @keyframes ask-ai-close { from { width: var(--ai-chat-width); } to { width: 0px; } }
      `}</style>
...
export function AISearchPanelList({ className, style, ...props }: ComponentProps<'div'>) {
  ...
  return (
    ...
    >
      {messages.length === 0 ? (
        <SuggestionChips sendMessage={chat.sendMessage} />
      ) : (
