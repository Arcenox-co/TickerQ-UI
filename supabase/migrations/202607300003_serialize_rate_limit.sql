-- Serialize submissions for the same pseudonymous rate key so first-use
-- requests cannot race on the primary-key insert.
...
begin
  ...
  perform pg_advisory_xact_lock(pg_catalog.hashtextextended(p_rate_key, 0));
